...
<div class="container">
    ...
    <form class="card col-8 px-3 m-auto" action="{{ route("admin.posts.update", $post->id) }}" method="POST">
    @csrf
    @method('PUT')
        <div class="my-1">
            <label for="post-name" class="form-label ps-2">Nome Post:</label>
            <input type="text" class="form-control" id="post-name" name="name" value="{{ old('name', $post->name) }}">
            @error("name")
        ...
        <div class="my-1">
            ...
            <input type="text" class="form-control" id="post-description" name="description" value="{{ old('description', $post->description) }}">
            @error("description")
                <div class="alert alert-warning">
                    {{ $message }}
                </div>
        ...
        <div class="my-1 d-flex justify-content-around">
            <button class="btn btn-success my-2 px-3" type="submit">Aggiorna</button>
